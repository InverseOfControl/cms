 ...
 */
@Service
public class BaoShangAirControlCheckImpl extends BaseSign<ResLoanContractSignVo> {
    //包银渠道产品小类编号
    @Value("#{env['prodSubNo']?:''}")
    ...
    private static final Log log = LogFactory.getLog(BaoShangAirControlCheckImpl.class);
    ...
    public boolean execute(ReqLoanContractSignVo reqLoanContractSignVo, Response<ResLoanContractSignVo> res) {
        log.info("*************************包银渠道风控审核开始*************************loanNo："+reqLoanContractSignVo.getLoanNo());
        //查询条件
        ...
        //查询出业务申请流水号
        BMSLoanExt extByLoanNo = ibmsLoanExtEntityDao.findLoanExtByLoanNo(paramMap);
        if (extByLoanNo == null) {
            log.info("***借款扩展表信息为空！****");
            setError(new BizException(CoreErrorCode.DB_RESULT_ISNULL, "借款扩展表信息为空"), res);
            return false;
        }
        //查询借款主表信息
        ...
        /**************推送失败可以重新推送给包银*******************************/
        if (!EnumConstants.BaoYinSatus._005.getCode().equals(extByLoanNo.getByState())) {
            /*****************基础报文参数配置ok************************************/
            JSONObject dataJson = new JSONObject();
            dataJson.put("prodSubNo", prodSubNo);
            dataJson.put("busNumber", extByLoanNo == null ? "" : extByLoanNo.getBusNumber());// 业务流水号
            ...
            if (!StringUtils.isEmpty(marriage)) {
                if (!CollectionUtils.isEmpty(tmAppContactInfos)) {
                    if (EnumConstants.MaritalStatus.MARRIED.getCode().equals(marriage)) {
            ...
            /******************影像资料文件地址列表参数配置--证大渠道必传（to do）*********************/
            //央行征信查询授权书
            Map<String, String> paramMapPic = new HashMap<>();
            paramMapPic.put("subclass_sort", "Q");//文件类型编号
            ...
            JSONObject jsonResQ = new JSONObject(httpResponse.getContent());
            if (!"000000".equals(jsonResQ.get("errorcode"))) {
                setError(new BizException(CoreErrorCode.FACADE_RESPONSE_FAIL, "pic目录下文件接口调用返回result为null"), res);
                return false;
            }
            //第三方数据查询授权书的电子版
            Map<String, String> paramMapPic2 = new HashMap<>();
            paramMapPic2.put("subclass_sort", "R");
            paramMapPic2.put("appNo", reqLoanContractSignVo.getLoanNo());
            paramMapPic2.put("operator", reqLoanContractSignVo.getServiceName());//操作人姓名
            paramMapPic2.put("jobNumber", reqLoanContractSignVo.getServiceCode());//工号
            HttpResponse httpResponse2 = baoShangHttpService.baoShangPicFile(paramMapPic2);
            JSONObject jsonResR = new JSONObject(httpResponse2.getContent());
            if (!"000000".equals(jsonResR.get("errorcode"))) {
                setError(new BizException(CoreErrorCode.FACADE_RESPONSE_FAIL, "pic目录下文件接口调用返回result为null"), res);
                return false;
            }
            //身份证明材料
            Map<String, String> paramMapPic3 = new HashMap<>();
            paramMapPic3.put("subclass_sort", "B");
            paramMapPic3.put("appNo", reqLoanContractSignVo.getLoanNo());
            paramMapPic3.put("operator", reqLoanContractSignVo.getServiceName());//操作人姓名
            paramMapPic3.put("jobNumber", reqLoanContractSignVo.getServiceCode());//工号
            HttpResponse httpResponse3 = baoShangHttpService.baoShangPicFile(paramMapPic3);
            JSONObject jsonResB = new JSONObject(httpResponse3.getContent());
            if (!"000000".equals(jsonResB.get("errorcode"))) {
                setError(new BizException(CoreErrorCode.FACADE_RESPONSE_FAIL, "pic目录下文件接口调用返回result为null"), res);
                return false;
            }
            JSONArray jsonQArray = jsonResQ.getJSONArray("result");//央行征信查询授权书
            JSONArray jsonRArray = jsonResR.getJSONArray("result");  //第三方数据查询授权书的电子版
            JSONArray jsonBArray = jsonResB.getJSONArray("result");  //身份证明材料
            JSONArray imgNameList = new JSONArray();//影像资料文件地址列表参数配置
            if (jsonQArray.length() > 0) {
                try {
                    for (int i = 0; i < jsonQArray.length(); i++) {
                        ...
                        if ("N".equals(jb.getString("ifWaste"))) {//是否作废
                            String url = PICUtils.encryptAES("http:" + jb.getString("url"), byAesKey);
                            jsonImgList.put("imageUrl", byPicGateWayUrl + URLEncoder.encode((url), "UTF-8"));// 个人征信查询授权协议扫描版原件照片地址
                            jsonImgList.put("imageType", "06");
                            imgNameList.put(jsonImgList);
                        }
                    }
                } catch (Exception e) {
                    log.error("获取机构央行征信查询授权书照片地址加密异常",e);
                    setError(new BizException(BizErrorCode.EOERR, "获取机构央行征信查询授权书照片地址加密异常"), res);
                    return false;
                }
            } else {
                setError(new BizException(CoreErrorCode.DB_RESULT_ISNULL, "央行征信查询授权书为空"), res);
                return false;
            }
            if (jsonRArray.length() > 0) {
                try {
                    for (int i = 0; i < jsonRArray.length(); i++) {
                        JSONObject jsonImgList = new JSONObject();
                        JSONObject jb = new JSONObject(jsonRArray.get(i).toString());
                        if ("N".equals(jb.getString("ifWaste"))) {//是否作废
                            String url = PICUtils.encryptAES("http:" + jb.getString("url"), byAesKey);
                            jsonImgList.put("imageUrl", byPicGateWayUrl + URLEncoder.encode((url), "UTF-8"));// 个人信息使用和第三方机构数据查询授权书扫描版原件照片地址
                            jsonImgList.put("imageType", "08");
                            imgNameList.put(jsonImgList);
                        }
                    }
                } catch (Exception e) {
                    log.error("获取机构第三方数据查询授权书图片地址加密异常",e);
                    setError(new BizException(BizErrorCode.EOERR, "获取图片地址加密异常"), res);
                    return false;
                }
            } else {
                setError(new BizException(CoreErrorCode.DB_RESULT_ISNULL, "第三方数据查询授权书为空"), res);
                return false;
            }
            if (jsonBArray.length() > 0) {
                try {
                    for (int i = 0; i < jsonBArray.length(); i++) {
                        JSONObject jsonImgList = new JSONObject();
                        JSONObject jb = new JSONObject(jsonBArray.get(i).toString());
                        if ("N".equals(jb.getString("ifWaste"))) {//是否作废
                            String url = PICUtils.encryptAES("http:" + jb.getString("url"), byAesKey);
                            jsonImgList.put("imageUrl", byPicGateWayUrl + URLEncoder.encode((url), "UTF-8"));// 身份证明复印件
                            jsonImgList.put("imageType", "14");
                            imgNameList.put(jsonImgList);
                        }
                    }
                } catch (Exception e) {
                    log.error("获取身份证明材料图片地址加密异常",e);
                    setError(new BizException(BizErrorCode.EOERR, "获取身份证明材料图片地址加密异常"), res);
                    return false;
                }
            } else {
                setError(new BizException(CoreErrorCode.DB_RESULT_ISNULL, "身份证明材料为空"), res);
                return false;
            }
            dataJson.put("imgNameList", imgNameList);
            /******************渠道风控信息参数配置--证大渠道必传ok*********************/
            ...
            riskInfoJson8.put("riskCodeValue", bmsLoanAudit.getAccTerm());
            ...
            riskInfoJson10.put("riskCodeValue", appMain.getProductCd());
            ...
            riskInfoJson11.put("riskCodeValue", appMain.getAccLmt());
            ...
            riskInfoJson12.put("riskCodeValue", appMain.getAccLmt());
            ...
            riskInfoJson9.put("riskCodeValue", bmsLoanAudit.getSysCheckLmt());
            ...
            riskInfoJson13.put("riskCodeValue", appMain.getAccTerm());
            ...
            riskInfoJson14.put("riskCodeValue", bmsContractLoan == null ? 0.00 : bmsContractLoan.getMonthlyRepayment());
            ...
            riskInfoJson63.put("riskCodeValue", reslutMap.get("信息获取日期"));
            ...
            if (infos.has("data")) {
                ...
                //0500  表示包银返回推送成功
                if (jo.has("respcd") && "0500".equals(jo.get("respcd"))) {
                    //包银渠道审核中
                    paramExt.put("auditingState", EnumConstants.BaoyinAuditingState._000.getCode());//审核中（风控和合同签订）
                    paramExt.put("byState", EnumConstants.BaoYinSatus._005.getCode());//风控信息推送成功
                    paramExt.put("windControlDate", new Date());
                    res.setRepCode("000000");
                    res.setRepMsg(jo.get("resptx").toString());
                } else {
                    //推送包银失败
                    paramExt.put("byState", EnumConstants.BaoYinSatus._004.getCode());//风控信息推送失败
                    paramExt.put("windControlDate", new Date());
                    res.setRepCode("999999");
                    res.setRepMsg(byAirControlMsg+jo.get("resptx").toString());
                }
            } else {
                //推送包银失败
                paramExt.put("byState", EnumConstants.BaoYinSatus._004.getCode());//风控信息推送失败
                ...
                if (null != respDesc) {
                    res.setRepMsg(byAirControlMsg+respDesc);
                } else {
            ...
            if (updateBySatus != 1) {
                log.info("****更新包银返回的状态值失败****接口传入参数："+paramExt.toString());
